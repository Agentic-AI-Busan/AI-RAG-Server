...
COMMAND="python3 script/faiss_index_check.py data/$FILE_NAME vectordb/$REPO_NAME"

# tmux 세션 생성 및 명령 실행
tmux new-session -d -s $SESSION_NAME
tmux send-keys -t $SESSION_NAME "docker exec ai-preprocessing bash -c \"$COMMAND > logs/check_vector_db_logs 2>&1; exit \"" C-m