...
COMMAND="python3 script/create_restaurant_vectordb.py data/$FILE_NAME $REPO_NAME"

# tmux 세션 생성 및 명령 실행
tmux new-session -d -s $SESSION_NAME
tmux send-keys -t $SESSION_NAME "docker exec ai-preprocessing bash -c \"$COMMAND > logs/make_vector_db_logs 2>&1; exit \"" C-m